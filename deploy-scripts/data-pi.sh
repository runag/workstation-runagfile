...
data-pi::shellrcd::shell-aliases() {
  ...
  if [ ! -f "${outputFile}" ]; then
    ...
    local unlockCommand; unlockCommand="$(printf "%q" "echo Trying to unlock...; if findmnt -M ~/$(printf "%q" "${DATA_PI_DISK_NAME}") >/dev/null; then echo already unlocked; else if [ ! -b /dev/mapper/$(printf "%q" "${DATA_PI_DISK_NAME}") ]; then sudo cryptsetup luksOpen /dev/sda1 $(printf "%q" "${DATA_PI_DISK_NAME}") || { echo \"luksOpen error\" >&2; exit 1; }; fi; sudo fsck -pf /dev/mapper/$(printf "%q" "${DATA_PI_DISK_NAME}") || { echo \"fsck error\" >&2; exit 1; }; sudo mount /dev/mapper/$(printf "%q" "${DATA_PI_DISK_NAME}") ~/$(printf "%q" "${DATA_PI_DISK_NAME}") || { echo \"mount error\" >&2; exit 1; }; sudo swapon ~/kelly/swapfile || { echo \"swapon error\" >&2; exit 1; }; sudo systemctl start $(printf "%q" "${DATA_PI_SYNCTHING_SERVICE}@${DATA_PI_USER}").service || { echo \"syncthing service start error\" >&2; exit 1; }; echo done; fi")" || fail
